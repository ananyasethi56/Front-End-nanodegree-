...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
   ...
   // Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
    ...
    function changePizzaSizes(size) {
      switch(size) {
        case "1":
          newwidth = 25;//newwidth is assigning some value
          break;
        case "2":
          newwidth =33.33;
          break;
  ...
  var randomPizzas = document.querySelectorAll(".randomPizzaContainer");//declaring the variable outside the for loop so that it can be called by its Name


    for (var i = 0; i < randomPizzas.length; i++) {

      randomPizzas[i].style.width = newwidth + "%";//calling the random pizza by its variable name defined outside the loop

  }
}
//allover this work help to increase the speed of changing the size of pizza fastly for the convience of user
...
function updatePositions() {
  ...
  var phase = [];//making array of phase variable
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin((document.body.scrollTop / 1250) + i));//layout should be done first.
  }
 for(var k=0 ; k <items.length ; k++)
 {
    items[k].style.left = items[k].basicLeft + 100 * phase[k % 5] + 'px';//style has to be done later to increase the speed
  }
